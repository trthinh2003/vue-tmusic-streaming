...
const schema = yup.object({
	title: yup.string().required("Vui lòng nhập tiêu đề album."),
    artistIds: yup.string().required("Vui lòng chọn nghệ sĩ.")
});
...
// Watch for changes in props.newAlbum to populate form
watch(() => props.newAlbum, (newVal) => {
    if (newVal) {
        id.value = newVal.id || "";
        title.value = newVal.title || "";
        releaseDate.value = newVal.releaseDate ? dayjs(newVal.releaseDate) : null;
        artistIds.value = newVal.artist?.id || "";
        previewImageLocal.value = newVal.imageUrl || "";
        
        // Load current artist into artists list if not already there
        if (newVal.artist && !artists.value.find(a => a.id === newVal.artist.id)) {
            artists.value = [newVal.artist, ...artists.value];
        }
    }
}, { immediate: true, deep: true });

// Watch for changes in previewImage prop
...
onMounted(async () => {
    // Load initial artists list if needed
    try {
        const res = await getArtists(1, 20);
        artists.value = res.data.data;
    } catch (error) {
...
.album-modal :deep(.ant-select) {
	background: rgba(255, 255, 255, 0.1) !important;
}

.album-modal :deep(.ant-select .ant-select-selector) {
	background: rgba(255, 255, 255, 0.1) !important;
	border: 1px solid rgba(255, 255, 255, 0.2) !important;
	color: white !important;
}

.album-modal :deep(.ant-select .ant-select-selection-placeholder) {
	color: rgba(255, 255, 255, 0.4) !important;
}

.album-modal :deep(.ant-select .ant-select-arrow) {
	color: rgba(255, 255, 255, 0.5) !important;
}
