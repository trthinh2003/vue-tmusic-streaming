...
import AllTab from '@/components/client/ComponentTabsExploreModal/AllTab.vue';
import SongsTab from '@/components/client/ComponentTabsExploreModal/SongsTab.vue';
...
const emit = defineEmits(['update:modelValue', 'pause-play']);
...
// 4. Function để perform download
...
const loadDownloadStatuses = async (songs) => {
...
const {
  ...
  createNewPlaylist,
  loadPlaylists,
  handleSongAction: originalHandleSongAction // Rename để tránh conflict
} = useSongActions();
...
watch(() => filteredSongs.value, (newSongs) => {
  if (newSongs && newSongs.length > 0) {
    loadDownloadStatuses(newSongs);
  }
}, { immediate: true });
...
onMounted(async () => {
  try {
    ...
    await loadDownloadStatuses(filteredSongs.value);
