<template>
    <div class="song-create-container">
        ...
        <h1 class="page-title">Cập nhật bài hát</h1>
                        ...
                        <a-form-item>
                            <div class="lyrics-editor-container">
                                ...
                                <div class="editor-toolbar">
                                    ...
                                    <a-button 
                                        v-if="songData.LyricsFile"
                                        @click="useOriginalLyric"
                                        type="link"
                                        :loading="loadingLyric"
                                    >
                                        <redo-outlined /> Tải lại lyric gốc
                                    </a-button>
                                </div>
                            </div>
                        </a-form-item>
                        ...
                        <a-form-item label="File lyric (LRC hoặc TXT)">
                            <div class="lyrics-upload-container">
                                ...
                                <div v-if="songData.LyricsFile" class="file-info">
                                    <file-outlined />
                                    <span class="file-name">{{ songData.LyricsFile.split('/').pop() }}</span>
                                    <a :href="getCloudinaryRawUrl(songData.LyricsFile)" download class="ml-2">
                                        <download-outlined /> Tải về
                                    </a>
                                    <a :href="songData.LyricsFile" target="_blank" class="ml-2">
                                        <eye-outlined /> Xem trên Cloudinary
                                    </a>
                                </div>
                        ...
                        <a-form-item name="songFile">
                            <div class="song-upload-container">
                                ...
                                <div v-if="songData.SongFile" class="file-info">
                                    <file-outlined />
                                    <span class="file-name">{{ songData.SongFile.split('/').pop() }}</span>
                                    <a :href="songData.SongFile" target="_blank" class="ml-2">
                                        <eye-outlined />
                                    </a>
                                </div>
...
/************************** Fetch lyric **************************/
const fetchLyricContent = async (url) => {
    try {
        const rawUrl = url.includes('upload/') 
            ? url.replace('/upload/', '/upload/fl_attachment/')
            : url;
        
        const response = await axios.get(`${rawUrl}?t=${Date.now()}`, {
            responseType: 'text',
        ...
        if (response.data) {
            if (response.data.trim().startsWith('<!DOCTYPE html>') || 
                ...
                throw new Error('Cloudinary trả về HTML thay vì nội dung file');
            }
            
            songData.value.LyricsText = response.data;
            songData.value.LyricsFileContent = response.data;
            return true;
        }
        return false;
    } catch (error) {
        console.error('Không thể tải nội dung lyric', error);
        message.error(`Lỗi khi tải lyric: ${error.message}`);
        return false;
    }
};
...
const getCloudinaryRawUrl = (url) => {
    ...
    if (url.includes('res.cloudinary.com')) {
        ...
        if (!url.includes('fl_attachment')) {
            newUrl = url.replace('/upload/', '/upload/fl_attachment/');
        }
        
        // Thêm timestamp để tránh cache
        return `${newUrl}?t=${Date.now()}`;
    }
...
const fetchSongData = async () => {
    try {
        ...
        const songRes = await getSong(slug);
        ...
        songData.value = {
            ...
            Genres: data.genre ? data.genre.split(', ').map(name => {
                const found = genres.value.find(g => g.label === name);
                return found ? found.value : null;
            }).filter(Boolean) : [],
            Albums: data.albumId || '',
            Cour: data.description || '',
            Image: data.background,
            Cover: data.cover,
            SongFile: data.audio,
            Tags: data.tags ? data.tags.split(',').map(tag => tag.trim()) : [],
            LyricsText: '',
            LyricsFile: data.lyric
        };

        if (data.lyric) {
            const loaded = await fetchLyricContent(data.lyric);
            if (!loaded) {
                message.warning('Không thể tải nội dung lyric từ URL');
            }
        }
        
        previewImage.value = data.background;
        previewCover.value = data.cover;
        
    } catch (error) {
        message.error('Lỗi khi tải dữ liệu bài hát');
        console.error(error);
    }
};

const useOriginalLyric = async () => {
    if (!songData.value.LyricsFile) {
        message.warning('Không có lyric gốc để khôi phục');
        return;
    }

    loadingLyric.value = true;
    try {
        const rawUrl = getCloudinaryRawUrl(songData.value.LyricsFile);
        ...
        if (response.data.trim().startsWith('<!DOCTYPE html>') || 
            ...
            throw new Error('Cloudinary vẫn trả về HTML');
        }

        songData.value.LyricsText = response.data;
        message.success('Khôi phục lyric gốc thành công');
    } catch (error) {
        console.error('Lỗi khi khôi phục lyric:', error);
        
        try {
            const fallbackUrl = `${songData.value.LyricsFile}?force_download=true&t=${Date.now()}`;
            const fallbackResponse = await axios.get(fallbackUrl, {
                responseType: 'text'
            });
            
            songData.value.LyricsText = fallbackResponse.data;
            message.success('Đã khôi phục lyric bằng phương pháp dự phòng');
        } catch (fallbackError) {
            message.error('Không thể tải lyric gốc. Vui lòng thử upload lại file mới.');
        }
    } finally {
